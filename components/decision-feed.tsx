  ...
  return (
    <div className="surface-paper divide-y divide-line overflow-hidden">
      <div className="px-5 py-3 flex items-center justify-between">
        ...
        <span className="text-[11px] text-light">streamed via SSE · logged on-chain</span>
      </div>
      {items.map((d) => {
        const tint = KIND_TINT[d.kind];
        return (
          <div key={d.id} className="px-5 py-3 flex items-start gap-3 text-[13.5px]">
            <span className="mono text-[11px] text-light w-10 pt-1 shrink-0">{relTime(d.at)} ago</span>
            <span
            ...
            <span className="mono text-[11px] w-20 pt-1 shrink-0" style={{ color: "var(--vs-machine-deep)" }}>
              {d.agentName}
            </span>
            <span className="text-ink-2 flex-1 leading-relaxed">
              ...
              {d.llm && (
                <span
                  className="ml-2 mono text-[9px] px-1.5 py-0.5 rounded-md align-middle"
              ...
              {variant === "global" && (
                <span className="ml-2 text-light mono text-[10px]">→ {d.duelId}</span>
              )}
            </span>
            {d.txHash && (
              <span className="mono text-[10px] text-light shrink-0 pt-1 hidden sm:inline">
